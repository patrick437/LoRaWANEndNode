...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # If cars are detected, add them to the buffer
            if car_detections:
                detection_buffer.extend(car_detections)
                print(f"Added {len(car_detections)} car detections to buffer")
                
            # Transmit detection data if it's time
            if current_time - last_transmit_time >= TRANSMIT_INTERVAL and detection_buffer:
                print(f"Transmitting {len(detection_buffer)} car detections")
                
                # Create payload with car count and detection data
                payload = bytes([len(detection_buffer)])  # First byte is count
                
                # Add each car's detection data
        ...
        if detection_buffer:
            ...
            for detection in detection_buffer:
                try:
                    ...
                    print(f"Error encoding detection: {e}")
            try:
                lorawan_packet = prepare_lorawan_packet(payload)
                send_data(lorawan_packet)
            except Exception as e:
